...
par1 = ureal('dMdq',dMdq,'PlusMinus',3*dMdq_sigma);
par2 = ureal('dMdu',dMdu,'PlusMinus',3*dMdu_sigma);
par3 = ureal('Iyy',Iyy,'PlusMinus',3*Iyy_sigma);
...
delay = tf(1,1,'ioDelay', 0.005);

%Tunable regulators
Cq = ltiblock.pid('Cq','pid');   % tunable PID
Cq.Kp.Value = Kpq;               % initialize Kp
Cq.Ki.Value = Kiq;               % initialize Ki
Cq.Kd.Value = Kdq;               % initialize Kd
Cq.Tf.Value = Tf;                % set parameter Tf
Cq.u = 'e_q'; Cq.y = 'deltaM';
Ctheta = ltiblock.pid('Ctheta','pd');
Ctheta.Kp.Value = KPP;           % initialize Kp
Ctheta.Kd.Value = KPD;           % initialize Kd
Ctheta.Tf.Value = Tf;            % set parameter Tf
Ctheta.u = 'e_{Theta}'; Ctheta.y = 'q_0';

%Connect these components to build a model of the entire closed-loop 
%control system
Linner = Gq*mixer*ss(Cq);
% InnerLoop = feedback(Linner,1);
InnerLoop = feedback(Gq*motor*mixer*ss(Cq),1);
InnerLoop.u = 'q_0';
L = Gtheta*InnerLoop*ss(Ctheta);
CL = feedback(L,1);
CL.u = 'Theta_0'; CL.y = 'Theta';
