...
int idle = 54;            //Min value (at 55 the brushless starts to rotate)
...
void setup() {
  ...
  for (i = 0; i < 200; i++) {
    throttle[i+200] = 50;
    time[i+200] = millis() - t0;
    BL.writeMicroseconds(1000 + 10 * throttle[i+200]);
    delay(1);
    thrust[i+200] = SF * ReadINA(INA_Pin);
    }
  for (i = 0; i < 200; i++) {
    throttle[i+400] = 10;
    time[i+400] = millis() - t0;
    BL.writeMicroseconds(1000 + 10 * throttle[i+400]);
    delay(1);
    thrust[i+400] = SF * ReadINA(INA_Pin);
    }
  BL.writeMicroseconds(1000);      
