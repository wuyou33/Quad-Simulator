function x0 = findstates(sys,u,y,t,p,type)
...
%  'K' specifies that K does not dependent on scheduling, or 'CD' specifies
...
if isct(sys) % Continuous models
    ...
    x0 = lsqnonlin(@residue,ones(Nx,1),[],[],options,u(1:3*Nx,:),yr(1:3*Nx,:),t(1:3*Nx,:),p(1:3*Nx,:),a,b,c,d,type);
else
    % Get the system matrices
    [Altv,Bltv,Cltv,Dltv] = lpv2ltv(a,b,c,d,p(1:3*Nx,:));
    [HU,Gamma] = lift(Altv,Bltv,Cltv,Dltv,3*Nx);
    Y = y(1:3*Nx,:)';
    Y = Y(:);
    U = u(1:3*Nx,:)';
    U = U(:);
